...
def validate_mp_input_decorator(min_val: Union[float, str] = "0.0",
    ...
    def decorator(func: FuncType) -> FuncType:
        ...
        def wrapper(*args, **kwargs):
            ...
            # Check if the first argument is 'self' (instance of a class)
            is_method = args and hasattr(args[0], func.__name__) and args[0].__class__.__name__ in func.__qualname__
...
class BayesLogicHP:
    ...
    def calculate_posterior_probability(self, prior_a: MP_Float, prior_b: MP_Float,
        ...
        return max(mpmath.mpf(0), min(mpmath.mpf(1), posterior)) # Asegurar resultado en [0, 1]
    ...
    def calculate_conditional_probability(self, joint_probability: MP_Float,
        ...
        return max(mpmath.mpf(0), min(mpmath.mpf(1), conditional))
...
class StatisticalAnalysisHP:
    ...
    def calculate_cosines(entropy: MP_Float, prn_object: MP_Float) -> Tuple[MP_Float, MP_Float, MP_Float]:
        ...
        entropy = max(entropy, epsilon_mpf)
        prn_object = max(prn_object, epsilon_mpf)
...
class PRN_HP:
    ...
    def adjust_influence(self, adjustment: Union[MP_Float, float, str]) -> None:
        ...
        new_influence = max(mpmath.mpf(0), min(mpmath.mpf(1), new_influence))
...
def calculate_e_mpmath(method: str = 'taylor',
    ...
    if method == 'taylor':
        ...
        # (1 + 1/n)^n para n grande. Usaremos 'iterations' as n.
        n = mpmath.mpf(iterations)
        if n <= 0: n = mpmath.mpf('1e6') # Default value if iterations is not useful
        base = one + mpmath.fdiv(one, n)
        e_result = mpmath.power(base, n)

    elif method == 'const':
        e_result = mpmath.exp(one) # Use mpmath internal constant

    else:
        raise ValueError("Invalid method. Use 'taylor', 'limit', or 'const'.")
...
# --- Adjusted Demonstration Functions ---
...
def run_statistical_analysis_hp_example():
    ...
    print(f"Absolute Difference:   {mpmath.nstr(diff, n=PRECISION_DPS)}")
...
def run_prn_hp_example():
    ...
    print(f"Combined PRN: {combined_prn}")
...
def main():
    if __name__ == "__main__":
     """Main function that runs all high-precision demonstrations."""
    main()
    print("===== QuoreMindHP Demonstration (High Precision) =====")
    print(f"Global mpmath precision set to: {mpmath.mp.dps} decimal digits")
