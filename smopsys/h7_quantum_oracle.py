...
from smopsys.golden_physics import PHI, PI, golden_operator, o_n_to_phase_fragment
...
class MetriplexOracle:
    ...
    def to_hilbert_oracle(self, fock_basis: FockBasis) -> Callable:
        """
        Convert classical oracle to quantum operator for Hilbert space.
        Uses phase fragments generated by momentum energy.
        """
        def quantum_oracle(state_vector: np.ndarray) -> np.ndarray:
            ...
            for idx, occupation in enumerate(fock_basis.basis_states):
                # Map occupation to momentum index
                p = self._occupation_to_momentum(occupation)
                energy = self.energy_map[p]
                
                # Apply phase rotation d_symp (conservative)
                # Phase fragment derived from energy normalization
    ...
    def _occupation_to_momentum(self, occupation: Tuple[int, ...]) -> int:
        ...
        return (total_occ % range_size) + self.p_min
